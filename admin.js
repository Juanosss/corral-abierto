...
// ==========================================
// FUNCIONES Y MODALES DEL CALENDARIO DE RODEOS
// ==========================================
window.openAdminCalendarModal = function() {
    const setVal = (id, val) => { const el = document.getElementById(id); if (el) el.value = val || ''; };
    setVal('form-cal-id-old', '');
...
window.editAdminCalendarEvent = function(id) {
    ...
    const setVal = (idEl, val) => { const el = document.getElementById(idEl); if (el) el.value = val || ''; };
    setVal('form-cal-id-old', ev.id);
    setVal('form-cal-nombre', ev.nombre);
    setVal('form-cal-lugar', ev.lugar);
    setVal('form-cal-asociacion', ev.asociacion);
    setVal('form-cal-tipo', ev.tipo);
    setVal('form-cal-estado', ev.estado);
    setVal('form-cal-fecha-inicio', ev.fecha_inicio);
    setVal('form-cal-fecha-fin', ev.fecha_fin);
    setVal('form-cal-hora-inicio', ev.hora_inicio);
    setVal('form-cal-hora-fin', ev.hora_fin);
    setVal('form-cal-descripcion', ev.descripcion);
    setVal('form-cal-afiche', ev.afiche_url);
...
window.saveAdminCalendarEvent = async function(e) {
    ...
    renderAdminCalendarTable();
    closeAdminCalendarModal();
    if (typeof window.showToast === 'function') window.showToast(`📅 Evento "${nombre}" guardado.`);
};
...
// ==========================================
// FUNCIONES Y MODALES DE CONTROL DE SOCIOS
// ==========================================
window.openUsuarioModal = function() {
    const setVal = (id, val) => { const el = document.getElementById(id); if (el) el.value = val || ''; };
    setVal('form-user-email-old', '');
    setVal('form-user-nombre', '');
    setVal('form-user-apellido', '');
    setVal('form-user-email', '');
    setVal('form-user-telefono', '');
    setVal('form-user-active', 'true');

    const titleEl = document.getElementById('modal-usuario-title');
    if (titleEl) titleEl.innerText = "Agregar Nuevo Socio";
...
window.editUsuario = function(email) {
    ...
    const setVal = (id, val) => { const el = document.getElementById(id); if (el) el.value = val || ''; };
    setVal('form-user-email-old', user.email);
    setVal('form-user-nombre', user.nombre || '');
    setVal('form-user-apellido', user.apellido || '');
    setVal('form-user-email', user.email || '');
    setVal('form-user-telefono', user.telefono || '');
    setVal('form-user-active', (user.active === true || user.active === 'true') ? 'true' : 'false');

    const titleEl = document.getElementById('modal-usuario-title');
    if (titleEl) titleEl.innerText = "✏️ Editar Datos de Socio";
    const overlay = document.getElementById('modal-usuario-overlay');
    if (overlay) overlay.classList.add('active');
};
...
window.saveUsuario = async function(e) {
    ...
    const idx = (usuariosList || []).findIndex(u => u.email === (oldEmail || email));
    if (idx >= 0) {
        usuariosList[idx] = userObj;
    } else {
    ...
    renderUsuariosTable();
    closeUsuarioModal();
    if (typeof window.showToast === 'function') window.showToast(`👤 Socio "${nombre}" guardado.`);
};
